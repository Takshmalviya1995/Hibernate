...
static  AnnotationConfiguration cfg=new AnnotationConfiguration().configure();
    ...
    public static void bugWithStatus()
    {
    	System.out.println("**********BUG WITH USERS**********");
    	...
    	for(Iterator itr = list.iterator(); itr.hasNext();)
    	{
    		...
    		System.out.println("Status txt :"+status.getStatustext());
    		System.out.println(" ");
    		
    		Bug bug1 = status.getStatusbug();
			System.out.println("Bug ID           :"+bug1.getBugId());
			System.out.println("Bug Title        :"+bug1.getBugTitle());
			System.out.println("Bug Description  :"+bug1.getDescription());
			System.out.println("Bug LastDate     :"+bug1.getLastDate());
    ...
    public static void userwithevent()
    {
    	System.out.println("**********USER WITH EVENT**********");
    	...
    	for(Iterator itr = list.iterator(); itr.hasNext();)
    	{
			...
			for(Iterator itr1 = users.getEventuser().iterator();itr1.hasNext();)
			{
				Event event = (Event)itr1.next();
    			System.out.println("Event Id        :"+event.getEventId());
    			System.out.println("Event Title     :"+event.getEventTitle());
			...
			/*for(Iterator itr2 = users.getUsersbugassign().iterator();itr2.hasNext();)
			{
				BugAssign bugassign1 = (BugAssign)itr2.next();
				System.out.println("Assign Id        :"+bugassign1.getAssignid());
				System.out.println(" ");
			}*/
    ...
    public static void selectMileStone()
    {
    	System.out.println("**********ADMIN WITH PROJECTS**********");
    	List list = session.createQuery("from MileStone").list();
    System.out.println("MileStone Id    ||    MileStone Name     ||       Start Date      ||        End Date       ||     User Id     ||     User Name        ||       Bug Id       ||");
    	for(Iterator itr = list.iterator(); itr.hasNext();)
    	{
    		MileStone milestone1 = (MileStone) itr.next();
			System.out.print(milestone1.getMilestoneId()+"                   ");
